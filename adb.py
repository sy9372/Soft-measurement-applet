...
def ADB(lr,num,x_train, y_train,x_test,y_test):
    adb = AdaBoostRegressor(learning_rate=lr, loss='linear', n_estimators=num)
    ...
    return y_pred, adb, a, b, score

def ADBS(path, lr, num):
    ...
    y_pred, adb, a, b, score = ADB(lr, num, x_train, y_train, x_test, y_test)
    name1 = str(name + '+adb+' + str(int(score)) +'分+'+str(n)+'输入' + timestr)
    dirs = name1
    if not os.path.exists(dirs):
        os.makedirs(dirs)
    joblib.dump(adb, dirs+"/adb_model.joblib")
    ...
    def picture():
        ...
        plt.legend()
        plt.savefig("picture/adb_figure.png")
        plt.savefig(dirs + "/adb_figure.png")
        plt.close
