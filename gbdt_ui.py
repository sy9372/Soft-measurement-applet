...
class gbdtui(QMainWindow):
    ...
    def setupUi(self, widget):
        ...
        Form.setFixedSize(801, 584)
        widget.setStyleSheet("background-image:url(:/picture1.png)")
        self.label = QtWidgets.QLabel(widget)
        self.label.setGeometry(QtCore.QRect(320, 0, 141, 31))
        font = QtGui.QFont()
        font.setFamily("微软雅黑")
...
"")
        ...
        self.label_7.setGeometry(QtCore.QRect(50, 150, 81, 21))
        font = QtGui.QFont()
        font.setFamily("Agency FB")
        font.setPixelSize(16)
        font.setBold(True)
...
"")
        ...
        self.output_numBox_2.setGeometry(QtCore.QRect(160, 90, 51, 22))
        self.output_numBox_2.setObjectName("output_numBox_2")
        self.label_8 = QtWidgets.QLabel(widget)
        self.label_8.setGeometry(QtCore.QRect(280, 120, 131, 21))
        font = QtGui.QFont()
        font.setFamily("Agency FB")
        font.setPixelSize(16)
        font.setBold(True)
...
"color: rgb(255, 85, 0);")
        ...
        self.iter_lineEdit.setGeometry(QtCore.QRect(150, 150, 71, 21))
...
"background-color: rgb(85, 170, 255);")
        ...
        self.label_14.setGeometry(QtCore.QRect(270, 550, 191, 31))
        font = QtGui.QFont()
        font.setFamily("Agency FB")
...
"")
        ...
        # 设置按钮
        self.select_button.clicked.connect(self.select1)  # 点击选择文件按钮
        self.run_button.clicked.connect(self.select3)  # 点击开始按钮

        self.retranslateUi(widget)
        QtCore.QMetaObject.connectSlotsByName(widget)

    def select1(self):
        self.fileName1, self.filetype = QtWidgets.QFileDialog.getOpenFileName(None, "选取文件", "./")  # 设置文件扩展名过滤,注意用双分号间隔
        self.shujuji_Button.setText(self.fileName1)
        self.ljshurumoxingquanzhong = self.fileName1
        print(self.fileName1)  # 打印数据集文件路径


    def select3(self):
        name2 = os.path.basename(self.fileName1)
        t = time.strftime("-%Y%m%d-%H%M%S", time.localtime())  # 时间戳
        l.sys.stdout = l.Logger(name2 +'+gbdt' + t + '.txt')
        print(f"输入个数：{self.input_numBox.value()}" )  # 输入个数
        print(f"输出个数：{self.output_numBox_2.value()}")  # 输出结点数
        print(f"学习率：{self.lr_doubleSpinBox_2.value()}")  # 学习率
        print(f"迭代次数：{self.iter_lineEdit.text()}")  # 迭代次数
        print(f'叶子节点最小样本数：{self.minl_numBox_3.value()}')
        print(f'内部节点最小样本数：{self.input_numBox_4.value()}')
        print(f'学习器最大深度：{self.max_numBox_5.value()}')
        a, b, score = GBDTS(self.fileName1, float(self.lr_doubleSpinBox_2.value()), int(self.max_numBox_5.value()), int(self.minl_numBox_3.value()),
                int(self.input_numBox_4.value()), int(self.iter_lineEdit.text()))
        ...
        img = Image.open('picture/gbdt_figure.png')
        img.resize((427, 320))
        # img1.save('gbdt2.png')
        self.picture.setPixmap(QtGui.QPixmap('picture/gbdt_figure.png'))
        self.picture.setScaledContents(True)

    def retranslateUi(self, widget):
        _translate = QtCore.QCoreApplication.translate
        widget.setWindowTitle(_translate("widget", "GBDT"))
        self.label.setText(_translate("widget", "  GBDT算法"))
        ...
        self.label_10.setText(_translate("widget", "  内部节点最小样本数："))
