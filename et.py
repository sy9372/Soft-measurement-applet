...
def ET(num, max_num, mins_num, minl_num, x_train, y_train, x_test, y_test):
    et = RandomForestRegressor(n_estimators=num, max_depth=max_num, min_samples_split=mins_num, min_samples_leaf=minl_num, random_state=0)
    ...
    return y_pred, et, a, b, score

def ETS(path, num, max_num, mins_num, minl_num):
    ...
    y_pred, et, a, b, score = ET(num, max_num, mins_num, minl_num, x_train, y_train, x_test, y_test)
    name1 = str(name + '+et+' + str(int(score)) +'分+'+str(n)+'输入' + timestr)
    dirs = name1
    if not os.path.exists(dirs):
        os.makedirs(dirs)
    joblib.dump(et, dirs+"/et_model.joblib")
    ...
    def picture():
        ...
        plt.legend()
        plt.savefig("picture/et_figure.png")
        plt.savefig(dirs+"/et_figure.png")
        def get_desk_p():
