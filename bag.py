...
def BAG(num,x_train, y_train,x_test,y_test):
...
def BAGS(path,num):
    ...
    y_pred, bag, a, b, score = BAG(num, x_train, y_train, x_test, y_test)
    name1 = str(name + '+bag+' + str(int(score)) +'分+'+str(n)+'输入' + timestr)
    dirs = name1
    if not os.path.exists(dirs):
        os.makedirs(dirs)
    joblib.dump(bag, dirs+"/bag_model.joblib")
    ...
    def picture():
        ...
        plt.legend()
        # plt.show()
        plt.savefig("picture/bag_figure.png")
        plt.savefig(dirs + "/bag_figure.png")
        plt.close
