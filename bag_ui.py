...
class bagui(QMainWindow):
    ...
    def setupUi(self, Form):
        ...
        self.label_14.setGeometry(QtCore.QRect(305, 553, 191, 31))
        font = QtGui.QFont()
        font.setFamily("Agency FB")
        font.setPixelSize(15)
        font.setBold(True)
        ...
        self.label.setGeometry(QtCore.QRect(315, 5, 170, 31))
        font = QtGui.QFont()
        font.setFamily("微软雅黑")
    ...
    def select3(self):
        ...
        filenames = name3 + '+bag' + t
        l.sys.stdout = l.Logger(path1+f'./{filenames}.txt')
        print(f"输入个数：{self.input_numBox.value()}")
        print(f"输出个数：{self.output_numBox_2.value()}")
        print(f"估计器数量：{self.iter_lineEdit.text()}")
        a, b, score = BAGS(self.fileName1, int(self.iter_lineEdit.text()))
        self.mx_lineEdit.setText(str(a))
        self.r_lineEdit_2.setText(str(b))
        self.score_lineEdit_3.setText(str(int(score)))
        img = Image.open("picture/bag_figure.png")
        img.resize((427, 320))
        self.picture.setPixmap(QtGui.QPixmap("picture/bag_figure.png"))
        self.picture.setScaledContents(True)

    def retranslateUi(self, Form):
        _translate = QtCore.QCoreApplication.translate
        Form.setWindowTitle(_translate("Form", "Bagging"))
        ...
        self.label.setText(_translate("Form", "  Bagging算法"))
        self.label_7.setText(_translate("Form", "估计器数量："))
