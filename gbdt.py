...
def GBDT(lr,max_num,minl_num,mins_num,num,x_train, y_train,x_test,y_test):
    gbdt = GradientBoostingRegressor(learning_rate=lr, loss='absolute_error', max_depth=max_num, min_samples_leaf=minl_num,
min_samples_split=mins_num, n_estimators=num)
    gbdt.fit(x_train, y_train)
    y_pred = gbdt.predict(x_test)
    max_error = max(abs(y_pred - y_test) / abs(y_test))
    print('平均绝对误差：', mean_absolute_error(y_test, y_pred))
    print('平均绝对百分比误差：', mean_absolute_percentage_error(y_test, y_pred))
    ...
    return y_pred, gbdt, a, b, score

def GBDTS(path,lr,max_num,minl_num,mins_num,num):
    dataset_train, dataset_test, x_train, x_test, y_train, y_test = load_csv(path)
    name1 = os.path.basename(path)
    name = name1.split('.')[0]
    now = datetime.now()
    timestr = now.strftime("-%Y%m%d-%H%M%S")
    y_pred, gbdt,a, b, score = GBDT(lr, max_num, minl_num, mins_num, num, x_train, y_train, x_test, y_test)
    name1 = str(name + '+gbdt+' + str(int(score)) +'分+'+str(n)+'输入'+ timestr)
    dirs = name1
    if not os.path.exists(dirs):
        os.makedirs(dirs)
    joblib.dump(gbdt, dirs+"/gbdt_model.joblib")
    ...
    def picture():
        ...
        plt.legend()
        plt.savefig('picture/gbdt_figure.png')
        plt.savefig(dirs+"/gbdt_figure.png")
        plt.close
